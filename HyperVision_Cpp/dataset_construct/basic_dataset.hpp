...
public:
    ...
    // 添加 setter 方法来动态设置数据路径和标签路径
    void set_data_path(const string& path) {
        load_data_path = path;
    }

    void set_label_path(const string& path) {
        load_label_path = path;
    }
    ...
    void import_dataset(void) {
        ...
        _ifl.close();
        LOGF("p_label size: %lu\n", p_label->size());
        LOGF("p_parse_result size: %lu\n", p_parse_result->size());
