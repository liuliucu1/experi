...
auto traffic_graph::_pre_process_short(const unordered_set<size_t> & _short_index,
    ...
    // Check if we have a previous model to use for incremental learning
    if (has_trained_model && model_short_centroids.n_elem > 0) {
        LOGF("Using previous model centroids for short edge clustering (incremental learning)");
        
        // Use previous centroids as initial centroids for clustering
        mlpack::dbscan::DBSCAN<> k_short(us, vs);
        
        // Store the previous centroids
        arma::mat initial_centroids = model_short_centroids;
        
        // Make sure dimensions match
        if (initial_centroids.n_rows == dataset_short.n_rows) {
            // DBSCAN doesn't support initial centroids, so we'll use the standard method
            // but we'll use the parameters learned from the previous model
            k_short.Cluster<arma::mat>(dataset_short, assignments_short, centroids_short);
            
            // Update the model centroids for future incremental learning
            model_short_centroids = centroids_short;
        } else {
            LOGF("Dimension mismatch in short edge centroids, falling back to standard clustering");
            mlpack::dbscan::DBSCAN<> k_short_fallback(us, vs);
            k_short_fallback.Cluster<arma::mat>(dataset_short, assignments_short, centroids_short);
            
            // Update the model centroids for future incremental learning
            model_short_centroids = centroids_short;
        }
    } else {
        // Standard clustering without incremental learning
        mlpack::dbscan::DBSCAN<> k_short(us, vs);
        k_short.Cluster<arma::mat>(dataset_short, assignments_short, centroids_short);
        
        // Store the centroids for future incremental learning
        model_short_centroids = centroids_short;
...
auto traffic_graph::_pre_process_long(const unordered_set<size_t> & _long_index,
    ...
    // Check if we have a previous model to use for incremental learning
    if (has_trained_model && model_long_centroids.n_elem > 0) {
        LOGF("Using previous model centroids for long edge clustering (incremental learning)");
        
        // Use previous centroids as initial centroids for clustering
        mlpack::dbscan::DBSCAN<> k_long(ul, vl);
        
        // Store the previous centroids
        arma::mat initial_centroids = model_long_centroids;
        
        // Make sure dimensions match
        if (initial_centroids.n_rows == dataset_long.n_rows) {
            // DBSCAN doesn't support initial centroids, so we'll use the standard method
            // but we'll use the parameters learned from the previous model
            k_long.Cluster<arma::mat>(dataset_long, assignments_long, centroids_long);
            
            // Update the model centroids for future incremental learning
            model_long_centroids = centroids_long;
        } else {
            LOGF("Dimension mismatch in long edge centroids, falling back to standard clustering");
            mlpack::dbscan::DBSCAN<> k_long_fallback(ul, vl);
            k_long_fallback.Cluster<arma::mat>(dataset_long, assignments_long, centroids_long);
